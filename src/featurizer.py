...
def change_darknet_config_params(cfg_path: Path, params_names, params):
    ...
    def find_index(cfg_lst, parameter):
        return [cfg_lst[0].index(i) for i in cfg_lst[0] if ((str(parameter)+'=' in i) and ("#" not in i)) or ((str(parameter)+' =' in i) and ("#" not in i))]
    
    cfg_list = from_darknet_config_file(cfg_path)
    for name in tqdm(params_names):
        idx = find_index(cfg_list, name)
        cfg_list[0][idx[0]] = f"{name}={params[name]}"

    return cfg_list


def policy_steps(max_batches, batch, subdiv, n_img_in_trainset):
    #pattern = [2.0, 2.0, 0.5, 0.5, 0.1, 2.0, 0.5, 0.5, 10]
    pattern = [1.5, 1.25, 0.75, 0.5, 0.25, 3.0]

    max_epochs = round((max_batches * batch * subdiv) / n_img_in_trainset) + 1
    butch_num = n_img_in_trainset / (batch * subdiv)

    steps = ",".join(str(round(butch_num * n)) for n in range(1, max_epochs, 15))
...
def main():
    ...
    print(f"Trainset length: {trainset_length}")

    if params["policy"] == "steps":
        policy_sgdr = policy_steps(params["max_batches"], params["batch"], params["subdivisions"], trainset_length)
